...
Exports:
  ...
  - build_dataset: numpy arrays → PyTorch tensors
  ...
  - predict_spectrum: run inference for an arbitrary surface feature vector
  - evaluate_spectrum: predict and optionally plot against ground-truth spectrum
"""
...
def evaluate_X0N_spectrum(model, feature_vector, label_vector, squarefree_Ns, N=1, K=20):
    ...
    if len(actual_r) < K:
        print(f"Warning: requested K={K}, but only {len(actual_r)} values are available.")
...
def X0N_LOOCV(features, labels, k_spectral=40, epochs=2000, width=256):
    ...
    for i in range(len(X_all)):
        ...
        # 3. Train model (fresh initialization)
        model = MLP(input_dim=X_train.shape[1], output_dim=k_spectral, width=width)
        opt = torch.optim.Adam(model.parameters(), lr=1e-3)
        loss_fn = nn.MSELoss()
 
        # Simple training loop without scheduler for speed inside the LOOCV loop
...
# =============================================================================
#  6.  Spectrum prediction and evaluation for arbitrary surfaces
# =============================================================================
 
def predict_spectrum(model, feature_vector):
    """
    Predict the spectrum for an arbitrary surface of the form X0(N) given its feature vector.
    Automatically adjusts input vector length to match the model input dimension.
    """
    expected_dim = model.net[0].in_features
 
    feat = np.array(feature_vector, dtype=np.float32)
    if len(feat) > expected_dim:
        feat = feat[:expected_dim]
    elif len(feat) < expected_dim:
        feat = np.pad(feat, (0, expected_dim - len(feat)), mode="constant")
 
    t_feat = torch.tensor(feat, dtype=torch.float32)
    if t_feat.ndim == 1:
        t_feat = t_feat.unsqueeze(0)
 
    model.eval()
    with torch.no_grad():
        prediction = model(t_feat).cpu().numpy().flatten()
...
def evaluate_spectrum(model, feature_vector, actual_r=None):
    """
    Predict and optionally evaluate the spectrum for a surface of the form X0(N).
    """
    pred = predict_spectrum(model, feature_vector)
    k = len(pred)
 
    print(f"\n--- Predicted First {k} Spectral Parameters for the X0(N) ---")
    print(pred)
 
    if actual_r is not None:
        print("\n--- Comparison with Actual Spectrum for the X0(N) ---")
 
        actual_r = np.asarray(actual_r, dtype=float)
        limit = min(len(pred), len(actual_r))
        pred_slice   = pred[:limit]
        actual_slice = actual_r[:limit]
 
        mse = np.mean((pred_slice - actual_slice) ** 2)
        print(f"Mean Squared Error: {mse:.6f}")
        ...
        plt.bar(indices - width_bar / 2, actual_slice, width_bar, label="Actual",    alpha=0.7)
        plt.bar(indices + width_bar / 2, pred_slice,   width_bar, label="Predicted", alpha=0.7)
